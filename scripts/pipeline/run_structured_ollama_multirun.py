#!/usr/bin/env python3
"""Chain N consecutive ``run_structured_ollama_single`` runs with random seeds.

Sibling of ``run_dspy_ollama_multirun.py``; only difference is the inner
single-run script — the structured-outputs variant routes every LLM call
through Ollama's OpenAI-compat endpoint with schema-enforced JSON.
...
Why this exists
---------------
``run_structured_ollama_single`` does one pass with the seed baked into
...
seeds — exactly what this script does, mirroring the DSPy sibling.

Usage
-----
    python scripts/pipeline/run_structured_ollama_multirun.py \\
        --model gptoss \\
        --folder dummy \\
        --dataset tcga \\
...
from run_structured_ollama_single import (  # noqa: E402
    DATASETS,
    MAX_RUN_SLOTS,
    UNIFIED_MODELS,
    run_with_args,
...
def build_inner_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    ``run_structured_ollama_single.run_with_args`` expects. ``run`` is
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument("--organs", nargs="*", default=None,
                    help="Forwarded to run_structured_ollama_single.")
