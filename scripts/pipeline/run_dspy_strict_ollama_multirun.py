#!/usr/bin/env python3
"""Chain N consecutive ``run_dspy_strict_ollama_single`` runs with random seeds.

Third sibling of ``run_dspy_ollama_multirun.py`` and
``run_structured_ollama_multirun.py``; only difference is the inner
single-run script — the dspy_strict variant routes every per-organ DSPy
``Predict`` call through Ollama's OpenAI-compat endpoint with
JSONAdapter-driven schema enforcement and a ChainOfThought reasoning
field.
...
llm/{model_slug}_dspy_strict/runNN/`` layout. The model-level
``_manifest.yaml`` is updated atomically by each inner call, so the
K-sweep accumulates cleanly into a single manifest with one entry per
run (each carrying its own seed, parse-error rate, and validity flag).

Why this exists
---------------
``run_dspy_strict_ollama_single`` does one pass with the seed baked into
``configs/dspy_ollama_<alias>.yaml`` (or the base default ``10``).
Running a stochastic K-sweep means a wrapper that draws fresh per-run
seeds — exactly what this script does, mirroring the DSPy / structured
siblings.

Usage
-----
    python scripts/pipeline/run_dspy_strict_ollama_multirun.py \\
        --model gptoss \\
        --folder dummy \\
        --dataset tcga \\
...
from run_dspy_strict_ollama_single import (  # noqa: E402
    DATASETS,
    MAX_RUN_SLOTS,
    UNIFIED_MODELS,
    run_with_args,
...
def build_inner_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    ``run_dspy_strict_ollama_single.run_with_args`` expects. ``run`` is
    left unset so ``pick_next_run`` rotates through free slots."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument("--organs", nargs="*", default=None,
                    help="Forwarded to run_dspy_strict_ollama_single.")
