"""Digital Registrar Compare / Consensus Tool — canonical-layout Streamlit app.

Parallel of :mod:`.compare_app` for the canonical eval layout (the one
emitted by ``scripts/data/gen_dummy_skeleton.py`` and consumed by
``scripts/eval/_common/paths.py``)::

    {base_dir}/data/{dataset}/
        reports/{n}/{case_id}.txt
        reports_without_preann/{n}/{case_id}.txt
        annotations/{annotator}/{n}/{case_id}.json
        annotations/gold/{n}/{case_id}.json          ← consensus output
        ...
    {base_dir}/results/predictions/{dataset}/...     ← evaluation source

Two modes:
  • Consensus   — A vs B vs Gold (editable). Saves bare ``{case_id}.json``
                  under ``annotations/gold/{n}/`` so the eval pipeline picks
                  it up.
  • Evaluation  — Reference annotator (any annotator + ``gold``) vs a
                  prediction tree under ``results/predictions/...``.
                  Read-only.

Default ``base_dir`` is read from ``REGISTRAR_ANNOTATE_BASE_DIR`` (set by
the ``registrar-compare-workspace`` / ``registrar-compare-dummy`` console
scripts).
"""
...
class CompareSampleRef:
    ...
    path_b: str
    path_gold: str
    path_machine: str | None  # only set in evaluation mode
...
def _build_compare_samples(
    ...
    suffix_b: str,
    *,
    machine_method: str | None = None,
    machine_model: str | None = None,
    machine_run_id: str | None = None,
) -> list[CompareSampleRef]:
    """Walk the canonical layout and produce a CompareSampleRef per case.

    Discovery uses ``list_samples(ws, suffix_a)`` so cases are anchored to
    annotator A's existence (matching the legacy behaviour). The B path,
    gold path, and optional machine path are computed via the canonical
    helpers in :mod:`.io_canonical`.
    """
    base = list_samples(ws, suffix_a)
    out: list[CompareSampleRef] = []
    for s in base:
        machine = None
        if machine_method:
            machine = prediction_path(
                ws.base_dir, ws.dataset, machine_method,
                machine_model, machine_run_id, s.n, s.sample_id,
            )
        out.append(CompareSampleRef(
            ...
            path_a=annotator_path(ws, suffix_a, s.n, s.sample_id),
            path_b=annotator_path(ws, suffix_b, s.n, s.sample_id),
            path_gold=gold_path(ws, s.n, s.sample_id),
...
def _reload_samples() -> tuple[bool, str]:
    ...
    if st.session_state.mode == "consensus":
        ...
        st.session_state.samples = _build_compare_samples(ws, a, b)
    else:
        ref = st.session_state.eval_ref_suffix
        if not ref:
            st.session_state.samples = []
            return False, "請選擇 Reference annotator。"
        method = st.session_state.eval_method
        if not method:
            st.session_state.samples = []
            return False, "請選擇要評估的 prediction (method/model/run)。"
        st.session_state.samples = _build_compare_samples(
            ws, ref, ref,
            machine_method=method,
            machine_model=st.session_state.eval_model,
            machine_run_id=st.session_state.eval_run_id,
        )
...
def _reload_from_base(base_dir: str) -> tuple[bool, str]:
    """Index ``{base_dir}/data/`` for the current annotation mode and pick a dataset."""
    ...
    dataset = (
        ...
        if st.session_state.dataset in datasets
        else datasets[0]
    )
...
def _on_file_change():
    ...
    if st.session_state.mode == "consensus":
        ...
        # Evaluation: B-slot is the model prediction JSON.
        machine_path = sample.path_machine or ""
        raw_b = load_json(machine_path)
        st.session_state.annotation_b = strip_meta(raw_b)
        rehydrate_sentinels(st.session_state.annotation_b, raw_b.get("_meta"))
        st.session_state.gold_annotation = {}
...
# ── Gold editable widget (mirrors compare_app._gold_input) ─────────────────────
...
def _save_gold() -> None:
    ...
    payload["_meta"]["annotation_mode"] = st.session_state.annotation_mode
...
def _annotation_mode_selector() -> None:
    options = list(MODES)
    labels = MODE_LABELS
    cur = st.session_state.annotation_mode
    ...
    if selected != st.session_state.annotation_mode:
        st.session_state.annotation_mode = selected
        if st.session_state.base_dir:
            ok, msg = _reload_from_base(st.session_state.base_dir)
            (st.sidebar.success if ok else st.sidebar.error)(msg)
        st.rerun()


def _dataset_selector() -> None:
    datasets = st.session_state.available_datasets
    if not datasets:
        return
    cur = st.session_state.dataset
    idx = datasets.index(cur) if cur in datasets else 0
    selected = st.sidebar.selectbox(
        "Dataset", options=datasets, index=idx, key="dataset_select",
    )
    if selected != st.session_state.dataset:
        st.session_state.dataset = selected
        ws = load_workspace(
            st.session_state.base_dir, selected, st.session_state.annotation_mode,
        )
        st.session_state.workspace = ws
        _reload_samples()
        st.rerun()
...
def _eval_pickers() -> None:
    ...
    idx = options.index(cur) if cur in options else 0
    chosen_ref = st.sidebar.selectbox(
        "Reference", options=options, index=idx,
        format_func=lambda s: labels.get(s, s), key="sel_eval_ref",
    )
    if chosen_ref != st.session_state.eval_ref_suffix:
        st.session_state.eval_ref_suffix = chosen_ref

    # Prediction picker — discovered from workspace/results/predictions/{dataset}/.
    base = st.session_state.base_dir
    dataset = st.session_state.dataset
    if not base or not dataset:
        st.sidebar.caption("Select a dataset first to discover predictions.")
        return
    triples = list_predictions(base, dataset)
    if not triples:
        st.sidebar.warning(
            f"No predictions found under "
            f"{os.path.join(base, 'results', 'predictions', dataset)}."
        )
        st.session_state.eval_method = None
        st.session_state.eval_model = None
        st.session_state.eval_run_id = None
        return

    def _label(t: tuple[str, str | None, str | None]) -> str:
        method, model, run_id = t
        parts = [method]
        if model:
            parts.append(model)
        if run_id:
    ...
    cur_triple = (
        st.session_state.eval_method,
        st.session_state.eval_model,
        st.session_state.eval_run_id,
    )
    cur_idx = triples.index(cur_triple) if cur_triple in triples else 0
    chosen = st.sidebar.selectbox(
        "Machine prediction", options=list(range(len(triples))),
        index=cur_idx, format_func=lambda i: _label(triples[i]),
        key="sel_eval_pred",
    )
    method, model, run_id = triples[chosen]
    if (method, model, run_id) != cur_triple or chosen_ref != st.session_state.eval_ref_suffix:
        st.session_state.eval_ref_suffix = chosen_ref
        st.session_state.eval_method = method
...
def render_sidebar() -> None:
    ...
    if (
        ...
        and st.session_state.base_dir
        and os.path.isdir(st.session_state.base_dir)
        and not st.session_state.workspace
    ):
        st.session_state.auto_loaded = True
        _reload_from_base(st.session_state.base_dir)
    ...
    samples = st.session_state.samples
    if not samples:
        st.sidebar.info("選擇 base 資料夾與資料集後載入樣本。")
        return
...
def _sample_badge(sample: CompareSampleRef) -> str:
    if st.session_state.mode == "consensus":
        ...
        if has_a or has_b:
            return "½"
        return "·"
    has_ref = os.path.exists(sample.path_a)
    has_machine = bool(sample.path_machine) and os.path.exists(sample.path_machine)
    if has_ref and has_machine:
        return "✓M"
    if has_ref:
        return "R"
    if has_machine:
...
def render_consensus_main() -> None:
    ...
    with col_status:
        if os.path.exists(sample.path_gold):
            st.success(f"Gold exists: {sample.path_gold}")
...
def render_evaluation_main() -> None:
    samples = st.session_state.samples
    if not samples:
        st.info("👈 Select reference + prediction + base folder to begin.")
        return
    sample = samples[st.session_state.sample_idx]
    a = st.session_state.annotation_a
    b = st.session_state.annotation_b
    ...
    machine_label = " / ".join(p for p in (method, model, run_id) if p)
    st.markdown(
        f"### Evaluation · {sample.sample_id}  "
        f"<span class='sec-summary'>"
        f"Reference=<b>{ref_label}</b> vs Machine=<b>{machine_label}</b> · "
    ...
    if not os.path.exists(sample.path_a):
        ...
        return
    if sample.path_machine and not os.path.exists(sample.path_machine):
        st.warning(f"Machine prediction missing for this sample: {sample.path_machine}")
...
def main() -> None:
    ...
    if not st.session_state.samples:
        st.title("🔍 Compare / Consensus (Canonical)")
...
def _repo_root() -> Path:
    # src/digital_registrar_research/annotation/compare_app_canonical.py → parents[3]
...
def main_cli_workspace():
    """Console-script entry for ``registrar-compare-workspace``."""
    _spawn_streamlit(_repo_root() / "workspace")


def main_cli_dummy():
    """Console-script entry for ``registrar-compare-dummy``."""
